import React from 'react';
import { View, StyleSheet, ScrollView } from 'react-native';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import { useTheme } from '../../context/ThemeContext';
import { Typography, Button, Surface, Icon } from '../../components/common';
import type { RootStackScreenProps } from '../../navigation/types';

export const NovelDetailPlaceholderScreen: React.FC<
  RootStackScreenProps<'NovelDetail'>
> = ({ route, navigation }) => {
  const { colors } = useTheme();
  const insets = useSafeAreaInsets();
  const { novelId } = route.params;

  // Consistent test pair mapping per user requirement:
  // - kimi-wa-boku-no-koukai-ln / volume-1-chapter-1
  // - btth / mtl/chapter-1
  const sampleChapterId =
    novelId === 'btth' ? 'mtl/chapter-1' : 'volume-1-chapter-1';

  return (
    <ScrollView
      style={[styles.scroll, { backgroundColor: colors.surfaceBackground }]}
      contentContainerStyle={[
        styles.container,
        { paddingTop: Math.max(insets.top, 16) + 16, paddingBottom: insets.bottom + 24 },
      ]}
    >
      <View style={styles.headerRow}>
        <Button
          variant="text"
          icon="arrow_back"
          accessibilityLabel="Kembali ke layar sebelumnya"
          onPress={() => navigation.goBack()}
        />
        <Typography variant="headline" color="primary">
          Detail Novel
        </Typography>
      </View>

      <Surface variant="card" style={styles.card}>
        <Typography variant="title" color="primary">
          Parameter Rute
        </Typography>
        <Typography variant="body" color="secondary">
          ID Novel: {novelId}
        </Typography>

        <View style={styles.infoBanner}>
          <Icon name="info" size={20} color={colors.accentPrimary} />
          <Typography variant="caption" color="secondary" style={styles.infoText}>
            Layar ini berada di RootStack di luar MainTabs; bottom bar tersembunyi secara otomatis.
          </Typography>
        </View>

        <View style={styles.actionColumn}>
          <Button
            variant="filled"
            title={`Buka Bab (${sampleChapterId})`}
            icon="auto_stories"
            onPress={() =>
              navigation.navigate('Reader', {
                novelId,
                chapterId: sampleChapterId, // Forwarded intact without extra encoding/decoding
              })
            }
          />
          <Button
            variant="outlined"
            title="Kembali ke MainTabs"
            onPress={() => navigation.goBack()}
          />
        </View>
      </Surface>
    </ScrollView>
  );
};

const styles = StyleSheet.create({
  scroll: {
    flex: 1,
  },
  container: {
    paddingHorizontal: 20,
    gap: 16,
  },
  headerRow: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 8,
  },
  card: {
    width: '100%',
    padding: 16,
    gap: 12,
  },
  infoBanner: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 8,
    padding: 8,
    borderRadius: 8,
  },
  infoText: {
    flex: 1,
  },
  actionColumn: {
    gap: 10,
    marginTop: 8,
  },
});
